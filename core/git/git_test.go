...
func (gitSuite *gitTestSuite) TestListTreeForRegularCommit() {
	...
	gitSuite.Contains(tree.EntriesByPath, "src/fuse/java/com/dchealth/service/common", "no common dir")
	dirEntry = lookupNode(&tree.Entry, "src/fuse/java/com/dchealth/service/common")
	gitSuite.NotNil(dirEntry)
	gitSuite.True(dirEntry.IsDir)
	gitSuite.EqualValues(0, dirEntry.Size)
	gitSuite.Equal(7, len(dirEntry.EntriesByName))

	gitSuite.Contains(tree.EntriesByPath, "src/fuse/java/com/dchealth/service/common/YunUserService.java", "no java file")
	fileEntry := lookupNode(&tree.Entry, "src/fuse/java/com/dchealth/service/common/YunUserService.java")
	gitSuite.NotNil(dirEntry)
...
func (gitSuite *gitTestSuite) TestFileContents() {
	contents, err := gitSuite.provider.FileContents("2ca742044ba451d00c6854a465fdd4280d9ad1f5", "src/fuse/java/com/dchealth/service/common/YunUserService.java")
	gitSuite.Nil(err, "git.ListTree: %v", err)
...
func (gitSuite *gitTestSuite) TestFileContentsForNonExisting() {
	...
	contents, err = gitSuite.provider.FileContents("wat", "src/fuse/java/com/dchealth/service/common/YunUserService.java")
	gitSuite.NotNil(err)
	gitSuite.EqualValues(0, len(contents), "file contents size not as expected")
}
